...
window.ZoneDPoolManager = {
    ...
    render(containerEl) {
            ...
            tableRowsHtml = `
                <tr>
                    <td colspan="7" style="text-align:center; padding:30px; color:var(--text-muted); font-size:12px;">
                        No participants found matching current filters or pool is empty. Click "Import Excel/CSV" or "Add New" above.
                    </td>
                </tr>
            `;
        } else {
                ...
                tableRowsHtml += `
                    <tr style="${!isActive ? 'opacity:0.45; background:rgba(0,0,0,0.25);' : ''}">
                        ...
                        <td style="min-width:160px;">
                            <input type="text" value="${(p.name || '').replace(/"/g, '&quot;')}" placeholder="Participant Name..." 
                        ...
                        <td style="width:90px; text-align:center;">
                            <label class="ios-toggle-label" title="${isActive ? 'Active in Draw Pool' : 'Excluded from Draw Pool'}" style="justify-content:center;">
        ...
        if (currentRibbonMode === 'autohide') {
            ...
            headerControlsHtml = `
                <div onclick="ZoneDPoolManager.setRibbonMode('all')" title="Click to show pool controls" style="display:flex; align-items:center; justify-content:center; background:#101018; border:1px dashed #28283c; border-radius:4px; padding:5px; font-size:11px; color:#00e5a3; font-weight:700; cursor:pointer; transition:all 0.2s; flex-shrink:0;">
                    ▼ Auto-hide Active · Click to Reveal Pool Controls (${activeCount}/${totalCount} Active)
                </div>
            `;
            ...
            headerControlsHtml = `
                <div style="display:flex; align-items:center; justify-content:space-between; background:var(--bg-surface); padding:6px 10px; border-radius:6px; border:1px solid var(--border-light); gap:8px; flex-wrap:wrap; flex-shrink:0;">
                    ...
                    <div style="position:relative; flex:1; display:flex; align-items:center; min-width:200px;">
                        ...
                        <input type="text" placeholder="Search Full Name, ID / Ticket #..." value="${this.searchQuery.replace(/"/g, '&quot;')}" oninput="ZoneDPoolManager.setSearch(this.value)" style="width:100%; height:26px; background:#161622; border:1px solid #303044; padding:0 22px 0 26px; font-size:11px; border-radius:4px; color:#fff;">
                    ...
                    <div style="display:flex; align-items:center; gap:6px; flex-wrap:wrap;">
                        <div style="display:flex; background:#0b0b10; border:1px solid #282836; border-radius:4px; padding:1px; height:24px; align-items:center;">
                            ...
                            <button onclick="ZoneDPoolManager.setFilterStatus('active')" style="border:none; border-radius:3px; height:20px; padding:0 6px; font-size:10px; font-weight:700; cursor:pointer; ${this.filterStatus === 'active' ? 'background:var(--accent-cyan); color:#0c0c0c;' : 'background:transparent; color:#888;'}">Active</button>
                        ...
                        <button class="pool-action-btn secondary ribbon-display-trigger" onclick="Studio.openRibbonDisplayMenu(event)" title="Switch Ribbon Display Options" style="height:24px; padding:0 8px; font-size:10px; background:#181826; border-color:#36364c; color:#00e5a3;">
                            ...
                            <span>Ribbon Mode</span>
                        </button>

                        <button class="pool-action-btn accent" onclick="ZoneDPoolManager.addNewParticipant()" title="Add a new blank participant row" style="height:24px; padding:0 8px; font-size:10px;">
                            <svg class="svg-icon" viewBox="0 0 24 24" style="width:12px; height:12px;"><path d="M5 12h14M12 5v14"/></svg>
                            <span>+ Row</span>
                        </button>

                        <label class="pool-action-btn primary" title="Import Excel" style="height:24px; padding:0 8px; font-size:10px;">
                            <span>Import</span>
            ...
            headerControlsHtml = `
                ...
                <div class="pool-unified-header" style="flex-shrink:0;">
                    ...
                    <div style="display:flex; align-items:center; gap: 12px;">
                        <div style="width: 36px; height: 36px; border-radius: 8px; background: rgba(0, 229, 163, 0.1); border: 1px solid rgba(0, 229, 163, 0.3); display: flex; align-items: center; justify-content: center; color: var(--accent-cyan); flex-shrink: 0;">
                        ...
                        <div>
                            <div style="font-size: 11px; font-weight: 800; color: #fff; display: flex; align-items: center; gap: 6px;">
                                DRAW POOL ROSTER
                                <span style="background: rgba(0, 229, 163, 0.15); color: var(--accent-cyan); font-family: var(--font-mono); font-size: 10px; padding: 1px 6px; border-radius: 10px; border: 1px solid rgba(0, 229, 163, 0.3); font-weight:700;">${activeCount} Active</span>
                            </div>
                            <div style="font-size: 10px; color: var(--text-secondary); margin-top: 2px;">
                                Total Registered: <b style="color: #bbb;">${totalCount}</b> participants (${totalCount - activeCount} excluded)
                            </div>
                        </div>
                    </div>
                    ...
                    <div style="display:flex; align-items:center; gap: 12px; flex-wrap:wrap;">
                        ...
                        <div style="display:flex; background: #0b0b10; border: 1px solid #282836; border-radius: 6px; padding: 2px; height:28px; align-items:center;">
                            ...
                            <button onclick="ZoneDPoolManager.setFilterStatus('active')" style="border:none; border-radius:4px; height:22px; padding:0 10px; font-size:10px; font-weight:700; cursor:pointer; transition:all 0.2s; ${this.filterStatus === 'active' ? 'background:var(--accent-cyan); color:#0c0c0c; box-shadow:0 1px 3px rgba(0,229,163,0.3);' : 'background:transparent; color:#888;'}">Active</button>
                        ...
                        <div style="display:flex; gap: 6px; align-items:center;">
                            <button class="pool-action-btn secondary ribbon-display-trigger" onclick="Studio.openRibbonDisplayMenu(event)" title="Ribbon Display Options (Auto-hide / Show Tabs / Show All)" style="background:#181826; border-color:#36364c; color:#00e5a3;">
                                <svg class="svg-icon" viewBox="0 0 24 24"><rect width="18" height="18" x="3" y="3" rx="2"/><path d="M3 9h18"/><path d="M12 19V9"/></svg>
                                <span>Ribbon Mode</span>
                            </button>

                            <label class="pool-action-btn primary" title="Import Excel (.xlsx) or CSV spreadsheet file">
                                <svg class="svg-icon" viewBox="0 0 24 24"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4M7 10l5 5 5-5M12 15V3"/></svg>
                                <span>Import</span>
                                <input type="file" accept=".xls,.xlsx,.csv" style="display:none;" onchange="ZoneDPoolManager.handleExcelImport(event)">
                            </label>

                            <button class="pool-action-btn secondary" onclick="Studio.openTemplateOptionsModal()" title="Download standard Excel import template">
                                <svg class="svg-icon" viewBox="0 0 24 24"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4M17 8l-5-5-5 5M12 3v12"/></svg>
                                <span>Template</span>
                            </button>

                            <button class="pool-action-btn accent" onclick="ZoneDPoolManager.addNewParticipant()" title="Add a new blank participant row">
                                <svg class="svg-icon" viewBox="0 0 24 24"><path d="M5 12h14M12 5v14"/></svg>
                                <span>Add Row</span>
                            </button>

                            ${totalCount > 0 ? `
                            <button class="pool-action-btn danger" onclick="ZoneDPoolManager.clearAll()" title="Clear all participants from the pool">
                ...
                <div style="display:flex; align-items:center; justify-content:space-between; background: #0e0e14; padding: 8px 12px; border-radius: 6px; border: 1px solid #222230; gap: 10px; flex-wrap: wrap; flex-shrink:0;">
                    <div style="position:relative; flex: 1; display:flex; align-items:center; min-width: 260px;">
                        ...
                        <input type="text" placeholder="Search by Full Name, ID / Ticket #, or Department..." 
                    ...
                    <div style="display:flex; align-items:center; gap: 8px;">
                        <span style="font-size:10px; color:var(--text-secondary); font-weight:700;">FILTER CATEGORY:</span>
                    ...
                    <table class="winner-table" style="width:100%; border-collapse:collapse;">
                        <thead style="position:sticky; top:0; background:var(--bg-elevated); z-index:5; border-bottom:1px solid var(--border-light);">
                            <tr>
                                ...
                                <th style="width:130px; padding:8px 6px; color:var(--text-secondary); font-size:10px;">Category</th>
                                <th style="width:90px; text-align:center; padding:8px 6px; color:var(--text-secondary); font-size:10px;">Status (Active)</th>
                                <th style="width:60px; text-align:right; padding:8px 6px; color:var(--text-secondary); font-size:10px;">Action</th>
                            </tr>
                        </thead>
                        <tbody>
                            ${tableRowsHtml}
    ...
    clearAll() {
        if (!confirm("⚠️ Are you sure you want to CLEAR ALL participants from the pool? This action cannot be undone.")) return;
