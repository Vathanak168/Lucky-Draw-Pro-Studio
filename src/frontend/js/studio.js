...
window.Studio = {
    ...
    showReport() {
        ...
        if (results.length === 0 || !results.some(r => r)) {
            ...
            results.forEach((r, idx) => {
                if (!r) return;
                const roundType = r.type === 'list' ? 'Name' : ((r.type === 'id') ? 'ID / Ticket #' : 'Number');
    ...
    renderWordBackstageTab(tabId) {
        ...
        if (tabId === 'recent') {
            ...
            if (projects.length === 0) {
                rowsHtml = `<div style="padding:40px; text-align:center; color:var(--text-muted); font-size:13px;">No saved local projects yet. Create a New Project or click Save As!</div>`;
            } else {
                let listItems = '';
                projects.forEach(p => {
            ...
            contentArea.innerHTML = `
                <div style="display:flex; flex-direction:column; gap:20px;">
                    <div style="border-bottom:1px solid var(--border-light); padding-bottom:12px;">
                        <div style="font-size:18px; font-weight:800; color:#fff;">Create New Project or Template</div>
                        <div style="font-size:12px; color:var(--text-secondary);">Start with a blank workspace or choose an event template.</div>
                    </div>

                    <!-- Custom Blank Project Box -->
                    <div style="background:var(--bg-elevated); border:1px solid var(--accent-cyan); border-radius:var(--radius-md); padding:20px; display:flex; flex-direction:column; gap:12px; box-shadow:0 10px 30px rgba(0, 229, 163, 0.08);">
                        <div style="font-size:14px; font-weight:800; color:var(--accent-cyan); display:flex; align-items:center; gap:8px;">
                            <span>✨ Blank Project (Custom Event)</span>
                        </div>
                        <div style="display:flex; gap:12px;">
                            <input type="text" id="newWordProjNameInput" placeholder="Enter Project Name (e.g. Annual Gala 2026)" style="flex:1; font-size:14px; padding:10px 14px; background:#111; border:1px solid var(--border-light); color:#fff; border-radius:var(--radius-sm);">
                    ...
                    <div style="font-size:13px; font-weight:700; color:var(--text-secondary); margin-top:10px;">INSTANT EVENT TEMPLATES</div>
                    <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:16px;">
                        <div class="word-template-card" onclick="Studio.applyWordTemplate('gala')">
                            <div style="font-size:24px;">🏆</div>
                            <div style="font-weight:800; font-size:14px; color:#fff;">10-Round Gala Dinner</div>
                            <div style="font-size:11px; color:var(--text-secondary);">Includes Grand Prizes, VIP Draw, Regular & Consolation rounds pre-setup.</div>
                        </div>
                        <div class="word-template-card" onclick="Studio.applyWordTemplate('quick3')">
                            <div style="font-size:24px;">⚡</div>
                            <div style="font-weight:800; font-size:14px; color:#fff;">Quick 3-Round Draw</div>
                            <div style="font-size:11px; color:var(--text-secondary);">Fast-paced setup for 1st, 2nd, and 3rd place prizes with grid boxes.</div>
                        </div>
                        <div class="word-template-card" onclick="Studio.applyWordTemplate('single')">
                            <div style="font-size:24px;">🎁</div>
                            <div style="font-weight:800; font-size:14px; color:#fff;">Single Grand Prize</div>
                            <div style="font-size:11px; color:var(--text-secondary);">High suspense single number draw with dramatic glow and fanfare.</div>
                        </div>
                    </div>
                </div>
            `;
            ...
            contentArea.innerHTML = `
                <div style="display:flex; flex-direction:column; gap:20px;">
                    <div style="border-bottom:1px solid var(--border-light); padding-bottom:12px;">
                        <div style="font-size:18px; font-weight:800; color:#fff;">Open / Browse Project File from PC</div>
                        <div style="font-size:12px; color:var(--text-secondary);">Load a '.json' or '.ldp' Lucky Draw project file from your computer or flash drive.</div>
                    </div>

                    <label style="border:2px dashed var(--accent-cyan); border-radius:var(--radius-lg); padding:60px 20px; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:16px; cursor:pointer; background:rgba(0, 229, 163, 0.03); transition:all 0.2s;" onmouseover="this.style.background='rgba(0, 229, 163, 0.08)'" onmouseout="this.style.background='rgba(0, 229, 163, 0.03)'">
            ...
            contentArea.innerHTML = `
                <div style="display:flex; flex-direction:column; gap:20px;">
                    <div style="border-bottom:1px solid var(--border-light); padding-bottom:12px;">
                        <div style="font-size:18px; font-weight:800; color:#fff;">Save Project As...</div>
                        <div style="font-size:12px; color:var(--text-secondary);">Choose whether to save inside Studio local memory or download a '.ldp' file to PC.</div>
                    </div>
    ...
    selectCategoryForEdit(categoryName) {
        ...
        if (titleEl) titleEl.textContent = "✏️ Edit Category: " + categoryName;
    ...
    saveCategoryFromForm() {
        ...
        if (!name) {
            alert("Please enter a Prize Category Name.");
            return;
        }
    ...
    onCatDragOver(event) {
        ...
        if (trEl && !trEl.classList.contains('cat-drag-over')) {
            ...
            trEl.style.background = 'rgba(0, 229, 163, 0.1)';
        }
    },
    ...
    renderCategoryManagerTable() {
        ...
        if (categories.length === 0) {
            container.innerHTML = `<div style="padding:40px; text-align:center; color:#666; font-size:13px;">No categories found. Add one on the left!</div>`;
            return;
        }
            ...
            <table style="width:100%; border-collapse:collapse; text-align:left; font-size:13px;">
                <thead>
                    <tr style="background:#1a1a1a; border-bottom:1px solid #333; color:#aaa; font-size:11px; text-transform:uppercase;">
                        ...
                        <th style="padding:12px 14px; font-weight:800;">Category Name</th>
                        <th style="padding:12px 14px; font-weight:800;">Default Pool</th>
        ...
        categories.forEach((catName, idx) => {
            ...
            const poolText = (themeObj.defaultPool === 'list') ? 'Master Guest List (Name)' : ((themeObj.defaultPool === 'id') ? 'ID / Ticket # (ID Only)' : 'Numeric Range');
            ...
            html += `
                <tr draggable="true"
                    ...
                    onmouseover="this.style.background='rgba(0, 229, 163, 0.05)'"
                    onmouseout="this.style.background='${idx % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.015)'}'"
                    title="Drag row up or down to reorder category">
                    ...
                    <td style="padding:12px 14px; text-align:right;">
                        <div style="display:flex; justify-content:flex-end; gap:6px;">
                            <button class="btn-arena" onclick="Studio.selectCategoryForEdit('${catName.replace(/'/g, "\\'")}')" title="Edit Category" style="padding:6px 12px; font-size:11px; border-color:#444;">
                                ✏️ Edit
                            </button>
                            <button class="btn-arena btn-arena-danger" onclick="Studio.deleteCategoryFromManager('${catName.replace(/'/g, "\\'")}')" title="Delete Category" style="padding:6px 10px; font-size:11px;">
                                🗑️ Delete
                            </button>
                        </div>
                    </td>
    ...
    renderParticipantManagerTable() {
        ...
        if (filtered.length === 0) {
            container.innerHTML = `
                <div style="padding:40px; text-align:center; color:#666; font-size:13px;">
                    No participants found matching current filters. Click "+ Add" or "Import Excel/CSV" above.
                </div>
            `;
            return;
        }
            ...
            <table class="category-table" style="width:100%; border-collapse:collapse; text-align:left;">
                <thead>
                    <tr style="background:#1a1a1a; border-bottom:1px solid #282828; color:#aaa; font-size:11px; text-transform:uppercase; letter-spacing:0.5px;">
                        ...
                        <th style="padding:12px 14px; width:220px;">Department / Note</th>
        ...
        filtered.forEach((p, rowIdx) => {
            ...
            let badgeHtml = '';
            if (p.hidden) {
                badgeHtml = `<span style="background:rgba(255,255,255,0.08); border:1px solid #555; color:#aaa; padding:3px 10px; border-radius:12px; font-size:10px; font-weight:700;">DISABLED</span>`;
            } else if (isWon) {
                badgeHtml = `<span style="background:rgba(245,158,11,0.15); border:1px solid #f59e0b; color:#f59e0b; padding:3px 10px; border-radius:12px; font-size:10px; font-weight:700;">🏆 WON PRIZE</span>`;
            } else {
            ...
            html += `
                <tr style="border-bottom:1px solid #222; background:${rowIdx % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.015)'}; transition:all 0.2s;"
                    onmouseover="this.style.background='rgba(0, 229, 163, 0.04)'"
                    ...
                    <td style="padding:10px 14px; text-align:right;">
                        <div style="display:flex; justify-content:flex-end; gap:6px;">
                            ...
                            <button class="btn-arena" onclick="Studio.toggleParticipantStatus(${p.originalIndex})" title="${p.hidden ? 'Enable' : 'Disable'}" style="padding:4px 10px; font-size:11px; border-color:${p.hidden ? 'var(--accent-cyan)' : '#444'}; color:${p.hidden ? 'var(--accent-cyan)' : '#bbb'};">
    ...
    editParticipantPrompt(idx) {
        ...
        const p = list[idx];
        if (!p) return;
        const newId = prompt('Enter ID / Ticket #:', p.id || p.ticket || '');
        if (newId === null) return;
        const newName = prompt('Enter Full Name:', p.name || '');
        if (newName === null || !newName.trim()) return;
        const newDept = prompt('Enter Department / Company / Note:', p.dept || p.company || '');
    ...
    deleteParticipantConfirm(idx) {
        ...
        const p = list[idx];
        if (!p) return;
        if (!confirm(`Delete "${p.name || p.id}" from the master participant pool?`)) return;
        list.splice(idx, 1);
        S.saveParticipantList(list);
        Draw.initializePoolsSilently();
        this.renderParticipantManagerTable();
    ...
    clearAllParticipantsConfirm() {
        if (!confirm('WARNING: Are you sure you want to permanently clear ALL participants from this project?')) return;
        ...
        reader.onload = (e) => {
            try {
                ...
                if (existing.length > 0) {
                    const choice = confirm(`Found ${parsedList.length} participant(s) in "${file.name}".\n\nClick OK to MERGE (add to existing ${existing.length} participants).\nClick CANCEL to REPLACE ALL existing data with this file.`);
                    if (choice) {
                ...
                Draw.initializePoolsSilently();
                alert(`✅ Successfully loaded ${parsedList.length} participant(s)!`);
