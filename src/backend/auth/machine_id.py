...
def get_hardware_fingerprint() -> str:
    ...
    Generates a deterministic, tamper-resistant Machine ID string (e.g. LDP-849A-2F1B).
    """
    ...
    # Hash and format nicely for display: e.g., LDP-849A-2F1B
    sha = hashlib.sha256(raw_id.encode('utf-8')).hexdigest().upper()
    part1 = sha[0:4]
    part2 = sha[4:8]
    return f"LDP-{part1}-{part2}"
