...
class GatekeeperService:
    ...
    def _load_session(self):
        """Loads machine-bound authentication token if valid and not expired (12h limit)."""
        if os.path.exists(AUTH_TOKEN_FILE):
            try:
                with open(AUTH_TOKEN_FILE, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                # Verify that token belongs to THIS machine and is within 12 hours
                if data.get("machine_id") == self.machine_id and data.get("unlocked") is True:
                    timestamp = float(data.get("timestamp", 0))
                    if time.time() - timestamp < 43200: # 12 hours lifetime
